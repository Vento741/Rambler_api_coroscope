    ...
    def _prepare_user_message(self, calendar_data: Dict[str, Any], user_type: str) -> str:
        ...
        if user_type == "free":
            ...
            return (
                ...
                f"ЗАПРЕЩЕНО: Подтверждать задание, отвечать на вопросы, начинать свой ответ с 'я понял', 'Приступаю', 'Сейчас посмотрю' и т.д. и т.п. А также использовать форматы ответов, которые не подходят для сообщений в телеграмме. Не использовать звёздочки в ответе! \n"
                f"Дата: {calendar_data['date']}\n"
                f"Фаза луны: {calendar_data['moon_phase']}\n\n"
                f"Лунные дни:\n{moon_days_text}\n\n"
                f"Рекомендации:\n{recommendations_text}"
